...
from backend.models.models import Profile, ProfileQuestion, User, Job, Application
...
class DugoutChatRequest(BaseModel):
    message: str


@router.post("/profiles/{profile_id}/dugout-chat")
async def dugout_chat(profile_id: int, req: DugoutChatRequest, db: Session = Depends(get_db)):
    """AI coaching chat in conversational format — like a manager giving advice between innings."""
    profile = db.query(Profile).filter(Profile.id == profile_id).first()
    if not profile:
        raise HTTPException(404, "Profile not found")
    ...
    if profile.profile_summary:
        profile_context += f"\nProfile Summary: {profile.profile_summary[:500]}"

    # Gather job search stats
    total_jobs = db.query(Job).filter(Job.profile_id == profile_id).count()
    liked_jobs = db.query(Job).filter(Job.profile_id == profile_id, Job.status == "liked").count()
    passed_jobs = db.query(Job).filter(Job.profile_id == profile_id, Job.status == "passed").count()
    pending_jobs = db.query(Job).filter(Job.profile_id == profile_id, Job.status == "pending").count()

    total_apps = db.query(Application).filter(Application.profile_id == profile_id).count()
    ...
    offer_apps = db.query(Application).filter(
        Application.profile_id == profile_id,
        Application.pipeline_status.in_(["offer", "accepted"]),
    ).count()
    rejected_apps = db.query(Application).filter(
        Application.profile_id == profile_id,
        Application.pipeline_status == "rejected",
    ).count()

    # Recent job titles (last 5 liked or applied)
    recent_jobs = db.query(Job).filter(
        Job.profile_id == profile_id,
        Job.status.in_(["liked", "applied"]),
    ).order_by(Job.created_at.desc()).limit(5).all()
    recent_titles = [f"{j.title} at {j.company}" for j in recent_jobs]

    stats_context = f"""Job Search Stats:
- Total jobs scouted: {total_jobs}
- Pending review: {pending_jobs}
- Liked/shortlisted: {liked_jobs}
- Passed: {passed_jobs}
- Applications submitted: {total_apps}
- Currently interviewing: {interview_apps}
- Offers received: {offer_apps}
- Rejections: {rejected_apps}"""

    if recent_titles:
        stats_context += f"\n- Recent interests: {'; '.join(recent_titles)}"

    # Batting average
    at_bats = liked_jobs + passed_jobs
    avg = (liked_jobs / at_bats) if at_bats > 0 else 0
    stats_context += f"\n- Batting average: {avg:.3f}"

    system_prompt = """You are a veteran baseball manager coaching a player through their job search. Use baseball metaphors naturally. Be warm, encouraging, and specific based on their actual profile and search data. Keep responses to 2-3 short paragraphs."""
...
The player says: {req.message}

Respond as their manager/coach:"""

    try:
        response = await ai_generate(prompt, max_tokens=600, model_tier="balanced", use_cache=False)
        if not response or not response.strip():
            raise HTTPException(500, "AI did not generate a response")
        return {"response": response.strip()}
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Dugout chat failed: {e}")
        raise HTTPException(500, f"Dugout chat failed: {str(e)}")
